  ...
  for (let i = 0, listNumber = 1; i < items.length; i += LIST_ITEM_SIZE) {
    ...
    try {
      ...
      if (err.status === 429 && retryCount < MAX_RETRIES) {
        console.log(`Retrying "${listName}" list - ${MAX_RETRIES - retryCount} attempts left`);
        retryCount++;
        await sleep(DELAY_BETWEEN_REQUESTS * retryCount); // Increase delay on each retry
        i -= LIST_ITEM_SIZE; // Retry the same chunk
        continue;
      } else {
